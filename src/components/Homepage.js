...
import {
  ...
  Grid, Button, TextField, Stack, FormControl, InputLabel, Select, MenuItem,
  FormGroup, FormControlLabel, Checkbox, Accordion, AccordionSummary, AccordionDetails
} from '@mui/material';
...
function Homepage() {
  ...
  useEffect(() => {
    const filterResults = catInfo.filter(cat => {
      ...
      const colorMatches = !colorFilter || cat.color.toLowerCase().includes(colorFilter);
      const breedMatches = !breedFilter || cat.breed.toLowerCase().includes(breedFilter);
      return nameMatches && genderMatches && ageMatches && colorMatches && breedMatches;
    });
                      ...
                      <Grid item xs={12} sm={6} md={3}>
                        <FormControl fullWidth>
                          <TextField
                            ...
                            variant="outlined"
                            value={colorFilter}
                            onChange={(e) => setColorFilter(e.target.value.toLowerCase())}
                          />
                        </FormControl>
                      </Grid>
                      ...
                      <Grid item xs={12} sm={6} md={3}>
                        <FormControl fullWidth>
                          <TextField
                            ...
                            variant="outlined"
                            value={breedFilter}
                            onChange={(e) => setBreedFilter(e.target.value.toLowerCase())}
                          />
                        </FormControl>
                      </Grid>
        ...
        <Container sx={{ py: 1 }} maxWidth="md">
          <Grid container spacing={4}>
            {filteredCatInfo.length > 0 ? (
              ...
              <Grid item xs={12}>
                <Typography variant="h6" align="center" style={{ margin: '20px 15px 10px 15px' }}>
                    Sorry. No cat found.
                </Typography>
            </Grid>
            )
